RSpec.describe Gitlab::Authenticate do
  let(:valid_data) do
    {
  ...
  describe '#execute' do
    it 'returns authenticated on valid token' do
      stub_request(:get, "https://gitlab.com/api/v4/user").to_return( body: { username: 'johndoe' }.to_json )
      stub_request(:get, "https://gitlab.com/api/v4/groups").to_return( body: '[]' )
    ...
    it 'returns not authenticated on invalid token' do
      stub_request(:get, "https://gitlab.com/api/v4/user").to_return( status: 403 )
